...
<div>
  ...
  <div>
    ...
    <HotelDetail :message='message'></HotelDetail>
    <!-- Popup实现 入住离店日期 -->
...
<script>
  ...
  import HotelDetail from '@/components/hotelRoomList/HotelDetail.vue'
  ...
  export default {
    ...
    methods: {
      ...
      loadHotelInfo () {
        ...
        fetchHotelInfoAxios(params).then(response => {
          if (response.data.Success == true) {
            ...
            this.message = response.data.Content.Description;
      ...
      loadRoomList() {
        // TODO 滚动加载
        this.$store.dispatch('getHotelRoomList');
      },
      // 内联日历关闭
      inlineCalClose() {
      ...
      setCheckOutDate(val) {
        ...
        this.$store.dispatch('setInlineCal', {value: false});
        this.$store.dispatch("hasRoomListLoading", true);
        this.$store.dispatch('getHotelRoomList') // TODO 下拉加载更多功能实施
      },
      outdate(val){
        this.$store.dispatch('setCheckOut', {value: val});
      },
